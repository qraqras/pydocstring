...
//!     fn visit_google_section(&mut self, source: &str, section: &GoogleSection<'_>) {
...
//! printer.visit_google_docstring(result.source(), &doc);
//! ```
...
pub trait DocstringVisitor: Sized {
    ...
    fn visit_google_docstring(&mut self, source: &str, doc: &GoogleDocstring<'_>) {
        walk_node(source, doc.syntax(), self);
    }
    /// Called for each Google section.
    fn visit_google_section(&mut self, source: &str, sec: &GoogleSection<'_>) {
        walk_node(source, sec.syntax(), self);
    }
    /// Called for each argument entry.
    fn visit_google_arg(&mut self, source: &str, arg: &GoogleArg<'_>) {
        walk_node(source, arg.syntax(), self);
    }
    /// Called for the Return entry in a Returns section, if present.
    fn visit_google_return(&mut self, source: &str, rtn: &GoogleReturn<'_>) {
        walk_node(source, rtn.syntax(), self);
    }
    /// Called for the Yield entry in a Yields section, if present.
    fn visit_google_yield(&mut self, source: &str, yld: &GoogleYield<'_>) {
        walk_node(source, yld.syntax(), self);
    }
    /// Called for each exception entry.
    fn visit_google_exception(&mut self, source: &str, exc: &GoogleException<'_>) {
        walk_node(source, exc.syntax(), self);
    }
    /// Called for each warning entry.
    fn visit_google_warning(&mut self, source: &str, wrn: &GoogleWarning<'_>) {
        walk_node(source, wrn.syntax(), self);
    }
    /// Called for each See Also item.
    fn visit_google_see_also_item(&mut self, source: &str, sai: &GoogleSeeAlsoItem<'_>) {
        walk_node(source, sai.syntax(), self);
    }
    /// Called for each attribute entry.
    fn visit_google_attribute(&mut self, source: &str, att: &GoogleAttribute<'_>) {
        walk_node(source, att.syntax(), self);
    }
    /// Called for each method entry.
    fn visit_google_method(&mut self, source: &str, mtd: &GoogleMethod<'_>) {
        walk_node(source, mtd.syntax(), self);
    }
    // ── NumPy ─────────────────────────────────────────────────────────────
    /// Called for the NumPy docstring root.
    fn visit_numpy_docstring(&mut self, source: &str, doc: &NumPyDocstring<'_>) {
        walk_node(source, doc.syntax(), self);
    }
    /// Called for the deprecation notice, if present.
    fn visit_numpy_deprecation(&mut self, source: &str, dep: &NumPyDeprecation<'_>) {
        walk_node(source, dep.syntax(), self);
    }
    /// Called for each NumPy section.
    fn visit_numpy_section(&mut self, source: &str, sec: &NumPySection<'_>) {
        walk_node(source, sec.syntax(), self);
    }
    /// Called for each parameter entry.
    fn visit_numpy_parameter(&mut self, source: &str, prm: &NumPyParameter<'_>) {
        walk_node(source, prm.syntax(), self);
    }
    /// Called for each Returns entry.
    fn visit_numpy_returns(&mut self, source: &str, rtn: &NumPyReturns<'_>) {
        walk_node(source, rtn.syntax(), self);
    }
    /// Called for each Yields entry.
    fn visit_numpy_yields(&mut self, source: &str, yld: &NumPyYields<'_>) {
        walk_node(source, yld.syntax(), self);
    }
    /// Called for each exception entry.
    fn visit_numpy_exception(&mut self, source: &str, exc: &NumPyException<'_>) {
        walk_node(source, exc.syntax(), self);
    }
    /// Called for each warning entry.
    fn visit_numpy_warning(&mut self, source: &str, wrn: &NumPyWarning<'_>) {
        walk_node(source, wrn.syntax(), self);
    }
    /// Called for each See Also item.
    fn visit_numpy_see_also_item(&mut self, source: &str, sai: &NumPySeeAlsoItem<'_>) {
        walk_node(source, sai.syntax(), self);
    }
    /// Called for each reference entry.
    fn visit_numpy_reference(&mut self, source: &str, r#ref: &NumPyReference<'_>) {
        walk_node(source, r#ref.syntax(), self);
    }
    /// Called for each attribute entry.
    fn visit_numpy_attribute(&mut self, source: &str, att: &NumPyAttribute<'_>) {
        walk_node(source, att.syntax(), self);
    }
    /// Called for each method entry.
    fn visit_numpy_method(&mut self, source: &str, mtd: &NumPyMethod<'_>) {
        walk_node(source, mtd.syntax(), self);
    }
}
...
pub fn walk_node<V: DocstringVisitor>(source: &str, node: &SyntaxNode, visitor: &mut V) {
    for child in node.children() {
        match child {
            ...
            SyntaxElement::Node(n) => match n.kind() {
                ...
                SyntaxKind::GOOGLE_SEE_ALSO_ITEM => visitor.visit_google_see_also_item(source, &GoogleSeeAlsoItem(n)),
